...
def main():
    ...
    # すでに処理済みの DisclosureNumber をセットとして取得
    processed_disclosures = set(df_financial["DisclosureNumber"])
    df_new_records = df_financial_pre[~df_financial_pre["DisclosureNumber"].isin(processed_disclosures)].copy()
    print(f"🧾 新しいレコード数: {len(df_new_records)} 件")
